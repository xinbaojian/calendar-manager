...
use calendarsync::mcp::{CalendarMCP, CreateEventParams, DeleteEventParams};
use rmcp::handler::server::wrapper::Parameters;
use std::sync::Arc;

#[tokio::test]
async fn test_mcp_delete_event() {
    // 使用内存数据库
    ...
    let event_id = mcp
        .create_event(Parameters(CreateEventParams {
            title: "待删除的会议".to_string(),
            description: Some("这个会议将被删除".to_string()),
    ...
    assert!(message.contains("删除成功"));
}

#[tokio::test]
async fn test_mcp_delete_event_not_found() {
    // 使用内存数据库
    let test_id = uuid::Uuid::new_v4();
    let db_path = format!("sqlite::file:///tmp/mcp-test-{}.db", test_id);
    let pool = create_pool(&db_path).await.unwrap();
...
#[tokio::test]
async fn test_mcp_delete_event_access_denied() {
    // 使用内存数据库
    let test_id = uuid::Uuid::new_v4();
    let db_path = format!("sqlite::file:///tmp/mcp-test-{}.db", test_id);
    let pool = create_pool(&db_path).await.unwrap();
    ...
    let result = mcp2.delete_event(Parameters(params)).await;
    assert!(result.is_err());
}

#[tokio::test]
async fn test_mcp_delete_event_empty_id() {
    // 使用内存数据库
    let test_id = uuid::Uuid::new_v4();
    let db_path = format!("sqlite::file:///tmp/mcp-test-{}.db", test_id);
    let pool = create_pool(&db_path).await.unwrap();
