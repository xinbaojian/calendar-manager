...
use calendarsync::mcp::{CalendarMCP, CreateEventParams, GetEventParams};
use rmcp::handler::server::wrapper::Parameters;
use std::sync::Arc;

#[tokio::test]
async fn test_mcp_get_event() {
    // 使用内存数据库
    ...
    let result = mcp.get_event(Parameters(params)).await;
    assert!(result.is_ok());

    let event_json = result.unwrap();
    let event: serde_json::Value = serde_json::from_str(&event_json).unwrap();
    assert_eq!(event["title"], "重要会议");
    assert_eq!(event["description"], "讨论项目进度");
    assert_eq!(event["location"], "会议室A");
}

#[tokio::test]
async fn test_mcp_get_event_not_found() {
    // 使用内存数据库
    let test_id = uuid::Uuid::new_v4();
    let db_path = format!("sqlite::file:///tmp/mcp-test-{}.db", test_id);
    let pool = create_pool(&db_path).await.unwrap();
...
#[tokio::test]
async fn test_mcp_get_event_access_denied() {
    // 使用内存数据库
    let test_id = uuid::Uuid::new_v4();
    let db_path = format!("sqlite::file:///tmp/mcp-test-{}.db", test_id);
    let pool = create_pool(&db_path).await.unwrap();
    ...
    let params = GetEventParams { id: event_id };

    let result = mcp2.get_event(Parameters(params)).await;
    assert!(result.is_err());
}

#[tokio::test]
async fn test_mcp_get_event_empty_id() {
    // 使用内存数据库
    let test_id = uuid::Uuid::new_v4();
    let db_path = format!("sqlite::file:///tmp/mcp-test-{}.db", test_id);
    let pool = create_pool(&db_path).await.unwrap();
    ...
    // 测试空 ID
    let params = GetEventParams { id: "".to_string() };
